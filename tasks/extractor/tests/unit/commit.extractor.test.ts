...
describe("Commit extractor", function () {
    ...
    describe("getJiraKeys", function () {
        ...
        it("should parse remote auto-merge commit without squash", async function () {
            // given
            commitMessageProviderMock.getCommitMessages.withArgs(dummyProject, dummyBuildId).resolves(["Merge remote-tracking branch 'remote/origin/master' into feature/JIE-540-test-branch"]);
        ...
        it("should parse simple commit", async function () {
            // given
            commitMessageProviderMock.getCommitMessages.withArgs(dummyProject, dummyBuildId).resolves(["JIE-540: sample commit message"]);
        ...
        it("should parse local branch merge commit", async function () {
            // given
            commitMessageProviderMock.getCommitMessages.withArgs(dummyProject, dummyBuildId).resolves(["Merge branch 'hotfix/JIE-540'"]);
        ...
        it("should parse remote auto-merge commit without squash with full-URL", async function () {
            ...
            // given
            commitMessageProviderMock.getCommitMessages.withArgs(dummyProject, dummyBuildId).resolves(["Merge branch 'master' of git@github.com:shark300/VSTSExtension-JiraIssueExtractor.git into feature/JIE-540-sample-commit-message"]);
        ...
        it("should parse remote auto-merge commit with pull request without squash to another branch and multiline commit message", async function () {
            ...
            // given
            commitMessageProviderMock.getCommitMessages.withArgs(dummyProject, dummyBuildId).resolves([`Pull request #11: feature/JIE-1223 sample commit message
...
* commit '7ebc2c0a2e9faf47c557c31a4c4362194985972b':
  ...
  JIE-225: sample commit message`]);
            ...
            // then
            expect(actual).to.have.members(["JIE-1223", "JIE-18", "JIE-1257", "JIE-225"]);
        });
...
JIE-225: sample commit message`]);
        ...
        it("should parse revert commit", async function () {
            ...
            // given
            commitMessageProviderMock.getCommitMessages.withArgs(dummyProject, dummyBuildId).resolves([`Revert "JIE-1223: sample commit message"
